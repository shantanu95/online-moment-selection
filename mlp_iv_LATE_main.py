...
def execute_strategy_iteration(
    ...
    nuisance_endpoints = np.arange(200, num_samples * 2, step=int(np.sqrt(num_samples)))
    ...
    elif strategy_name == "fixed_equal_true_nu":
        nuisance = OracleNuisanceEstimator()
        strategy = OracleStrategy(
            sample_revealer=sample_revealer,
            true_scm=true_scm,
    ...
    elif strategy_name == "fixed_equal_est_nu":
        nuisance = MLPNuisanceEstimator(
            horizon=num_samples, batch_endpoints=nuisance_endpoints
        )
        strategy = OracleStrategy(
